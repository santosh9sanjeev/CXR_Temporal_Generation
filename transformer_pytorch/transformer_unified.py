...
class TransformerLM_unified(nn.Module):
    def __init__(
        ...
        self.prev_att_emb = AbsolutePositionalEmbedding(dim, max_seq_len)  # max_seq_len = img_len * max_img_num + max_text_len
        self.curr_att_emb = AbsolutePositionalEmbedding(dim, max_seq_len)
    ...
    def forward(self, batch, causal, return_encodings=False, **kwargs):  # kwargs = {'mask': tensor with same shape x}
        img1, txt, modes, view, image_state = batch['img1'], batch['txt'], batch['modes'], batch['view_position'], batch['image_state']
        ...
        for i, x_img_slot in enumerate(imgs):
            ...
            for bsz in range(b):
                ...
                slot_img_state = image_state[i][bsz]
                # print('viewwwwwwwwwwwwww',slot_view, slot_img_state)

                if slot_view == 'AP_and_curr' and slot_img_state == '1':
                    # print('slot_view_ifffffffffffffffffffffffff', slot_img_state)
                    out = self.curr_att_emb(x_img[bsz:bsz + 1])  # out: [img_len, dim]
                elif slot_view == 'AP_and_prev' and slot_img_state == '0':
                    # print('slot_view_elseeeeeeeeeeeeeeeeeeeeeee', slot_img_state)
                    out = self.prev_att_emb(x_img[bsz:bsz + 1])  # out: [img_len, dim]
    ...
    def generate_texts(
        ...
        for cur_len in range(txt_seq_len - 1):
            batch['txt'] = out[:, image_seq_len:]
            logits,_ = self(batch, causal=causal)
